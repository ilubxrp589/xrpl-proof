...
import { buildTholos, PART, N_COLS } from './tholos.js';
...
export class Renderer {
  ...
  resize() {
    ...
    for (const k of ['scene', 'b0', 'b0b', 'b1', 'b1b', 'b2', 'b2b']) if (this[k]) this[k].free();
  ...
  frame(st) {
    ...
    const lampCol = st.mode === 3 ? [0.4, 0.28, 1.1] : [1.28, 1.16, 0.98];
    ...
    const dome = M.apply(tVPfull, M.apply(tModel, [0, tb.yDome + 9, 0]));
