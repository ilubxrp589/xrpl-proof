...
function frame(dt) {
  ...
  // the dome's rays are the one part that shows each ledger's check happen: they draw back
  // into the dome as a new ledger arrives, and spread again once its header hashes to what
  // was signed. A retreat, not a blink: the rest of the sheet holds still
  st.glory += (target.glory - st.glory) * (1 - Math.exp(-dt * (reduced ? 60 : target.glory > st.glory ? 1.2 : 3.5)));
