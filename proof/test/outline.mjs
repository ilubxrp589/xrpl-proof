// The failure path: the proof request is made to fail, so the account must stay
// an outline (the relay's word) with the reason, never lit. Saves the view.
// Usage: node test/outline.mjs [address] [outdir]
...
const out = process.argv[3] || '/tmp/proof-outline';
...
await sleep(4000);
const s = await ev('JSON.stringify((({show, account}) => ({show, account}))(window.__proof.state()))');
console.log('state', s, '| tag:', await ev(`document.getElementById('acct-tag').innerText.replace(/\\n/g, ' | ')`));
